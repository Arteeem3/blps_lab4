...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleIOException(java.io.IOException ex) {
        ...
        if (msg != null && (msg.contains("Broken pipe") || msg.contains("Connection reset") || msg.contains("Конец канала"))) {
            log.warn("Client disconnected: {}", msg);
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(new ErrorResponse("CLIENT_DISCONNECTED", "Client closed connection prematurely"));
        }
        log.error("I/O error", ex);
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(new ErrorResponse("IO_ERROR", "I/O error occurred"));
    }
